...
class AuthHelper(object):
    ...
    @staticmethod
    def check_login_by(request, email=None):
...
class api_auth():
    ...
    def wrap_action(self, action_, *args, **kwargs):
        ...
        api_key = None
        params = request.params if 'POST' in request.method else request.matchdict
        ...
        if self.admin_only:
            if self._check_admin_only(request):
                ...
                return {'error': "Not authorized for request admin."}

        if request.user is not None:
            if AuthHelper.check_login(request, username):
                # then we're good, this is a valid user for this url
                return action_(*args, **kwargs)
        print(request.matchdict.get(self.api_field))
        print(self.api_field)
        # get the user the api key belongs to
        if self.api_field in params:
            # we've got a request with url params
            api_key = params.get(self.api_field, None)
            username = params.get('username', username)
            sekret = params.get('sekret', None)
            print("key :{0}, sekret :{1} ".format(api_key, sekret))
        ...
        if username is not None or api_key is not None and sekret is not None:
            ...
            if request.user and (request.user.username == username or request.user.secret == sekret):
                return action_(*args, **kwargs)
